    ...
    <form @submit.prevent="applySearch">
      <div class="row g-3">
        <div class="col d-flex justify-content-start">
          ...
          <!-- 검색버튼을 눌렀을때 반응 -->
          <input type="text" class="form-control" v-model="inputSearch" style="width: 70%" />

          <button class="btn btn-primary ms-2 px-4" @click="applySearch">검색</button>
        </div>
        <div class="col-3">
...
const params = ref({
  ...
  _page: 1, //현재페이지 조회
  _limit: 3, //몇개씩 조회할건지
  title_like: '', //특정 컬럼뒤에 like를 넣고 문자를 넣게 되면 검색할 수 있음.
})

//검색 버튼 눌렀을때, 반응
const applySearch = () => {
  params.value.title_like = inputSearch.value
}
